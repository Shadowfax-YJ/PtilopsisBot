...
async def test_collects_zip_before_removing_group_source(tmp_path: Path) -> None:
    ...
    async with httpx.AsyncClient(
        ...
        assert len(group.messages[0]) <= 30
        assert "\n" not in group.messages[0]
        collector.close()
...
async def test_disabled_receipts_keep_archiving_cleanup_and_daily_report(tmp_path: Path) -> None:
    body = zip_bytes()
    group = FakeGroup()
    settings = Settings(
        group_id=123, data_dir=tmp_path, send_receipts=False,
        delete_grace_hours=0, min_free_gib=0,
    )
    async with httpx.AsyncClient(
        transport=httpx.MockTransport(lambda request: httpx.Response(200, content=body))
    ) as http:
        ...
        try:
            collector.register(
                Upload(123, "file-a", 102, 456, "run-20260905-120000-000001.zip",
                       len(body), 1788580800, live=True, source_kind="live")
            )
            await collector.process_once()
            assert collector.record(1)["status"] == "archived"
            assert group.message_attempts == 0
            # Re-enabling and retrying must not send a receipt for the muted upload.
            collector.settings = settings.model_copy(update={"send_receipts": True})
            ...
            report = await collector.send_report(date(2026, 9, 5))
            assert report is not None
            assert group.messages == [report]
        finally:
            collector.close()
...
async def test_does_not_delete_uncollected_files_or_when_disabled(
    ...
    async with httpx.AsyncClient(
        ...
        if case == "incomplete":
            assert collector.record(record_id)["last_error"] == (
                "Content-Length 与群文件声明大小不符"
            )
        elif case == "http_error":
            assert collector.record(record_id)["last_error"] == "HTTPStatusError HTTP 403"
...
@pytest.mark.parametrize(
    ("error", "expected"),
    [
        (CollectionError("根目录无法唯一定位源文件（匹配 2 项）"),
         "根目录无法唯一定位源文件（匹配 2 项）"),
        (ValueError("https://files.test/?token=secret"), "ValueError"),
        (RuntimeError("https://files.test/?token=secret"), "RuntimeError"),
    ],
)
async def test_failed_download_preserves_only_controlled_error_details(
    tmp_path: Path, caplog: pytest.LogCaptureFixture, error: Exception, expected: str,
) -> None:
    class BrokenLookup(FakeGroup):
        async def file_url(self, file_id: str, busid: int) -> str:
            raise error
    ...
    async with httpx.AsyncClient() as http:
        collector = Collector(
            ...
            api=BrokenLookup(), http=http,
        )
        try:
            collector.register(
                Upload(123, "file-a", 102, 456, "run-20260905-120000-000001.zip", 100, 1788580800)
            )
            await collector.process_once()
            assert collector.record(1)["status"] == "failed"
            assert collector.record(1)["last_error"] == expected
...
def test_legacy_valueerror_recovery_is_scoped_and_does_not_reset_future_budgets(
    tmp_path: Path,
) -> None:
    settings = Settings(group_id=123, data_dir=tmp_path)
    collector = Collector(settings)
    for index in range(1, 9):
        collector.register(
            Upload(123, f"napcat:{index}", 102, 456,
                   "run-20260905-120000-000001.zip", 100, 1788580800)
        )
    with collector.db:
        collector.db.execute(
            "UPDATE uploads SET status='failed',attempts=3,last_error='ValueError'"
        )
        collector.db.execute("UPDATE uploads SET status='archived',collected_at=1 WHERE id=2")
        collector.db.execute("UPDATE uploads SET collected_at=1 WHERE id=3")
        collector.db.execute("UPDATE uploads SET last_error='HTTPStatusError HTTP 403' WHERE id=4")
        collector.db.execute("UPDATE uploads SET group_id=999 WHERE id=5")
        collector.db.execute("UPDATE uploads SET file_id='other-source' WHERE id=6")
        collector.db.execute("UPDATE uploads SET status='invalid' WHERE id=7")
        collector.db.execute("UPDATE uploads SET deleted_at=1 WHERE id=8")
    before = {row["id"]: dict(row) for row in collector.db.execute("SELECT * FROM uploads")}
    collector.close()

    # Constructing a collector for a report preview must not modify the retry state.
    collector = Collector(settings)
    try:
        assert dict(collector.record(1)) == before[1]
        assert collector.recover_legacy_failures() == 1
        recovered = collector.record(1)
        assert recovered["status"] == "queued"
        assert recovered["attempts"] == 0
        assert recovered["next_attempt_at"] == 0
        assert recovered["last_error"] is None
        for index in range(2, 9):
            assert dict(collector.record(index)) == before[index]
        # Even a new opaque ValueError must retain its exhausted budget on restart.
        with collector.db:
            collector.db.execute(
                "UPDATE uploads SET status='failed',attempts=3,last_error='ValueError' WHERE id=1"
            )
    finally:
        collector.close()
    collector = Collector(settings)
    try:
        assert collector.recover_legacy_failures() == 0
        assert collector.record(1)["attempts"] == 3
        assert collector.record(1)["status"] == "failed"
    finally:
        collector.close()


async def test_report_deduplicates_data_and_thanks_every_successful_uploader(
    tmp_path: Path,
) -> None:
    body = zip_bytes()
    group = FakeGroup()
    ...
    async with httpx.AsyncClient(
        ...
        report = collector.report(date(2026, 9, 5))
        assert report is not None
        assert f"数据总量：1 个包，共 {len(body)} B" in report
        assert f"当日新增：1 个包，共 {len(body)} B" in report
        assert "感谢当日上传数据的 MAA 训练家：小张、小李。" in report
        collector.close()
...
def test_scan_corrects_day_and_conflicting_metadata_does_not_overwrite_identity(
    ...
    assert collector.record(first)["nickname"] == "新名字"
    assert collector.report(date(2026, 9, 5)) is None  # Queued files are not collected data.
...
async def test_corrupt_deflate_stream_is_retried_and_never_deleted(tmp_path: Path) -> None:
    stream = io.BytesIO()
    ...
    async with httpx.AsyncClient(
        ...
        assert record_id is not None
        await collector.process_once()
        assert collector.record(record_id)["status"] == "failed"
        assert group.files == {"file-a"}
        collector.close()
