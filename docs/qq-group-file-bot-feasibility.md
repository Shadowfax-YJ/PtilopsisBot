...
> 本文保留早期调研时的平台比较与架构建议，不是当前实现规格。用户因 LLBot 启动授权门槛确认换回 **NapCat**；实现通过列表上传信息登记、操作前重新取 ID、删源前重新读取并核对远端哈希来处理临时 ID，具体限制见 [README](../README.md)。为解决群文件空间不足，保存后自动清理群文件是首版核心功能。首版范围以[实现地图](../.scratch/qq-group-run-collector/map.md)为准；下文的旧选型结论、扩展能力、运维和备份建议不自动成为开发要求。
