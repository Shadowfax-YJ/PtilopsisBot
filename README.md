...
引用使用 NapCat 文件聊天消息的 `message_id`，不是文件通知里的临时 `file_id`。只有同一连接期间收到的文件消息，且上传者、文件名、大小一致、上传时间相差不超过 60 秒，能与根目录文件唯一对应时才回复。找不到对应消息或存在歧义时保持静默；实际引用气泡的显示由 QQ 客户端决定。消息引用只在内存中保留最近 1000 条，断线或重启清空，不追补跨重启的收包回复。
