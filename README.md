...
1. 按 [NapCat 官方 Windows 安装说明](https://napneko.github.io/guide/boot/Shell)安装并登录机器人 QQ，确保它已加入目标群且有群文件删除权限。
2. 在 NapCat 中启用 **OneBot 11 WebSocket 客户端 / 反向 WebSocket**，URL 为 `ws://127.0.0.1:8080/onebot/v11/`，token 与本机配置一致。连接方式见 [NoneBot OneBot 文档](https://onebot.adapters.nonebot.dev/docs/guide/setup/)。只需这个连接，无需另开 NapCat HTTP API。
3. 运行机器人：
...
收到事件即登记下载；连接、重连及每 30 分钟补扫根目录。下载失败每隔 5 分钟重试，合计最多 3 次；检查不通过或耗尽次数后看错误原因，处理完再 `retry`。列表达到 `root_file_count` 时会提示检查是否漏收，可按实际群文件数量调整。
...
用 Windows 任务计划程序新建一个开机或登录触发的任务，程序为 `powershell.exe`，参数为 `-NoProfile -ExecutionPolicy Bypass -File "D:\Projects\databot\start.ps1"`。设置“如果任务已在运行，不启动新实例”，并关闭默认的运行时长限制。NapCat 也需要启动并保持 QQ 登录。
...
当前本机尚无 NapCat，也没有真实游戏 ZIP 样本。代码和离线验证不能替代真实群验收：安装连接后，使用专门测试上传确认群文件被移除、本地 ZIP 可读、日报仍保留贡献；同时检查实际根目录列表是否完整。验收完成前，[地图](.scratch/qq-group-run-collector/map.md)中的真实环境事项保持待验收。
