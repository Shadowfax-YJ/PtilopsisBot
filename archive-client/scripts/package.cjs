...
async function main() {
  ...
  const outputs = await packager({
    dir: root, out: path.join(root, 'out'), name: 'Archive Subscriptions', executableName: 'ArchiveSubscriptions',
  ...
  const archive = `ArchiveSubscriptions-${version}-${platform === 'darwin' ? 'macOS-AppleSilicon' : 'Windows-x64'}-portable.zip`;
