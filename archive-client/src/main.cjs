...
app.setAppUserModelId('local.archive.subscriptions');
...
function startupEnabled() { return !smoke && app.getLoginItemSettings().openAtLogin; }
function snapshot() {
...
function emit() {
  ...
  if (tray) {
    tray.setToolTip(`Archive 订阅 · ${config.paused ? '已暂停' : engine?.running ? '正在下载 / 检查' : '等待更新'}`);
    tray.setContextMenu(Menu.buildFromTemplate([
      { label: '打开 Archive 订阅', click: show },
...
function setupIPC() {
  ...
  register('autostart', value => {
    if (!smoke) app.setLoginItemSettings({ openAtLogin: Boolean(value), path: process.execPath, args: ['--background'] });
    emit(); return startupEnabled();
  });
...
async function makeWindow() {
  window = new BrowserWindow({ width: 1080, height: 780, minWidth: 800, minHeight: 650, show: false,
    title: 'Archive 订阅', backgroundColor: '#f6f7f9', icon: path.join(assets, 'icon.png'), autoHideMenuBar: true,
...
async function smokeTest() {
  ...
  assert(await evaluate("document.body.innerText.includes('Archive 订阅')"), 'main window not rendered');
...
if (!app.requestSingleInstanceLock() && !smoke) { app.quit(); }
  ...
  app.whenReady().then(async () => {
    ...
    Menu.setApplicationMenu(process.platform === 'darwin' ? Menu.buildFromTemplate([
      { label: 'Archive 订阅', submenu: [{ label: '关于 Archive 订阅', role: 'about' }, { type: 'separator' }, { label: '退出并停止下载', accelerator: 'Cmd+Q', click: quit }] },
    ...
    if (smoke) {
      ...
      dialog.showErrorBox('Archive 订阅无法启动', safeError(err)); await engine?.close(); app.exit(1);
    }
  });
}
